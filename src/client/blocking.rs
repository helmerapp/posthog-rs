...
impl Client {
    pub fn capture(&self, event: Event) -> Result<(), Error> {
        ...
        let capture_endpoint = "i/v0/e/";
        let endpoint = self.options.api_endpoint.clone() + capture_endpoint;
        self.client
            .post(endpoint)
    ...
    pub fn is_feature_enabled(&self, config: FeatureFlagPayload) -> Result<bool, Error> {
        ...
        let response = self.make_decide_request(request);
    ...
    pub fn make_decide_request(&self, request: DecideRequestData) -> Result<DecideResponse, Error> {
        ...
        let res_text = res.text().map_err(|e| Error::Connection(e.to_string()))?;
